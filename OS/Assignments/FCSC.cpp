...
void generateRandomProcesses(int arrivalTime[], int burstTime[], int numProcesses)
{
    for (int i = 0; i < numProcesses; ++i)
    {
        arrivalTime[i] = rand() % 21;
        burstTime[i] = rand() % 21 + 1;
    }
}

void sortProcessesByArrival(int arrivalTime[], int burstTime[], int numProcesses)
{
...
void calculateAverages(int turnaroundTime[], int waitingTime[], int numProcesses)
{
...
{
    sortProcessesByArrival(arrivalTime, burstTime, numProcesses);
    ...
    calculateAverages(turnaroundTime, waitingTime, numProcesses);
}

void SJF_Preemptive(int arrivalTime[], int burstTime[], int numProcesses)
{
    ...
    while (completedProcesses < numProcesses)
    {
        ...
        // Add a Gantt entry for each time slice.
        if (!ganttChart.empty() && ganttChart.back().processId == shortest + 1)
        {
            ganttChart.back().endTime++; // Extend the current process's slot
        }
        else
        {
...
void RoundRobin(int arrivalTime[], int burstTime[], int numProcesses, int quantum) {
    ...
    vector<GanttEntry> ganttChart;
    bool isProcessInQueue[numProcesses] = {false};  // To check if a process is already in queue

    // Load initially arrived processes into the queue
    for (int i = 0; i < numProcesses; ++i) {
    ...
    while (!processQueue.empty()) {
        ...
        if (remainingTime[i] > quantum) {
            ...
            remainingTime[i] = 0;
            completionTime[i] = currentTime;  // Mark completion time
        }
...
cout << ganttChart.back().endTime << endl;  // Print the final end time
}
...
int main()
{
    ...
    srand(time(0));
    generateRandomProcesses(arrivalTime, burstTime, numProcesses);
    ...
    do
    {
        ...
        case 1:
            ...
            break;
        case 2:
            SJF_Preemptive(arrivalTime, burstTime, numProcesses);
            break;
        case 3:
